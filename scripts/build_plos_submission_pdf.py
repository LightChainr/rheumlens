...
OUTPUT = (
    ...
    / "submission"
    / "plos_computational_biology"
    / "Design_identifiability_v2.0.0-rc3_initial_submission.pdf"
)


def clean_inline(text: str) -> str:
    text = text.replace(
        r"\frac{Y^\top M_DY}{Y^\top M_1Y}",
        "(Y<super>T</super> M<sub>D</sub>Y)/(Y<super>T</super> M<sub>1</sub>Y)",
    )
    text = text.replace(r"\mathcal{I}_D", "<i>I</i><sub>D</sub>")
    text = text.replace(r"R^2", "R<super>2</super>")
    text = text.replace(r"\top", "<super>T</super>")
    text = text.replace(r"\sim", "~")
    text = text.replace(r"\beta", "beta")
    text = text.replace(r"\delta", "delta")
    text = text.replace(r"\rho", "rho")
    text = text.replace(r"\epsilon", "epsilon")
...
def page_number(canvas, doc) -> None:
    ...
    canvas.drawString(
        2 * cm,
        0.9 * cm,
        "Design-identifiability study v2.0.0-rc3 initial submission",
    )
...
def main() -> None:
    ...
    doc = SimpleDocTemplate(
        ...
        topMargin=1.8 * cm,
        bottomMargin=1.7 * cm,
        title="When study design predicts disease",
